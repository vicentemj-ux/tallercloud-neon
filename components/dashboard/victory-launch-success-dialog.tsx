    ...
    if (!url?.trim()) {
      return (
        <p className="text-center text-xs font-medium text-red-600" role="alert">
          No se pudo generar el código de firma
        </p>
      )
    }
...
type VictoryLaunchSuccessDialogProps = {
  ...
  /** Usa `repairId` + `folio` del payload recién creado vía localStorage + ventana de impresión */
  onPrintTicket: () => void
  onPrintLabel: () => void
  onSendWhatsApp: () => void
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          ...
          description: `Se registró un abono de $${parseFloat(abonoMonto).toLocaleString("es-MX")} en caja`,
          variant: "default",
        })
      } else {
      ...
      >
        {payload ? (
          <div className="rounded-[1.35rem] px-5 pb-6 pt-6 sm:px-6">
            ...
            {/* Sección de Gestión de Anticipo */}
            {!abonoExitoso ? (
