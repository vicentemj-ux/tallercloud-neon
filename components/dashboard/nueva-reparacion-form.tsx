...
import { getTallerPlanType, getTallerSettings, type TallerPlanTipo, type TallerSettings } from "@/lib/actions/settings"
...
interface NuevaReparacionFormProps {
  ...
  /** Cuando hay un ID de reparación cargado (edición), se muestra el botón AGREGAR DETALLES y se puede pre-llenar el formulario. */
  editingRepairId?: string | null
  /** Solo modo modal: notifica si hay cambios respecto al último punto guardado (cierre con confirmación). */
...
export function NuevaReparacionForm({
  ...
  /** Modal: envío silencioso para generar firma con ticket real en BD (misma ruta que edición). */
  ...
  /** Modal de éxito tras crear orden (folio + impresión / WhatsApp) */
  ...
  const showRegisterError = useCallback((detail: string) => {
    ...
    if (
      ...
      d =
        "Respuesta genérica del servidor. Revisa en Supabase que existan las columnas requeridas en `reparaciones`, que RLS permita el insert y mira los logs de la función `createRepair` en Vercel."
    }
  ...
  // Al abrir en modo edición, cargar datos del ticket para pre-rellenar
  useEffect(() => {
  ...
  /** Cierra el modal de éxito, resetea el formulario y notifica al padre (lista / cierre modal) */
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (
        ...
        showRegisterError(
          "Sin conexión no se pueden guardar los cambios del ticket. Espera a recuperar la señal o revisa tu red.",
        )
        setSaving(false)
        return
      }
      ...
      if (onSubmitProp) {
        ...
        if (effectiveEdit && out?.repairId) {
          ...
          setSuccessPayload({
            ...
            countryCode: getCodigoTelefono(shopSettings?.pais ?? "México"),
          ...
          showRegisterError("No se recibió el folio de la orden. Intenta de nuevo.")
        }
      } else {
        setSubmitted(true)
        setSaving(false)
      ...
      const errorMsg =
        ...
        raw.includes("omitted in production")
          ? "Error al guardar (el servidor no expuso el detalle en producción). Revisa sesión, columnas en BD o intenta sin fotos. En desarrollo verás el error real en la consola."
    ...
    try {
      ...
      if (slot <= 0) {
        toast({
          ...
          description: "Ya alcanzaste el máximo de 3 fotos por ticket.",
        })
        return
      }
      ...
      toast({
        ...
        description: "Intenta de nuevo o sube una imagen desde la galería.",
      })
    } finally {
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription className="text-slate-600">
            Guardando localmente. El ticket quedó en cola y se subirá cuando vuelva la conexión.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
          <Button
        ...
        <AlertDialogHeader>
          ...
          <AlertDialogDescription asChild>
            <p className="text-base leading-relaxed text-slate-800">
              <span className="font-semibold text-slate-900">❌ Error al Registrar:</span>{" "}
  ...
  if (isModal) {
    ...
    return (
      ...
      >
        ...
        {/* ── Cuerpo: scroll interno; el diálogo usa max-h sin hueco inferior extra ── */}
        <div className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto px-3 pb-0 pt-2 sm:px-3.5 sm:pt-2.5">
                ...
                <CardContent className="space-y-2 p-2.5 sm:p-3">
                  ...
                  <div className="flex items-start gap-2 rounded-lg border border-slate-200 bg-white px-2.5 py-2 shadow-sm">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] leading-snug text-slate-500">
                        Precio a definir tras diagnóstico. Deja en 0 el presupuesto.
                      </p>
                    </div>
                  </div>
                  ...
                  {ingresoACotizar ? (
                    <div className="rounded-md bg-amber-50 px-2.5 py-1.5 text-[11px] font-medium text-amber-900 ring-1 ring-amber-100 flex items-center gap-1.5">
                      ...
                      Pendiente de cotización — el precio se llenará después del diagnóstico
                    </div>
                  ) : null}
          ...
          {/* NOTE: La sección de costos se movió al componente MonitorUtilidadOperativa en la página de detalle */}
        ...
        <div className="sticky bottom-0 z-30 min-h-0 shrink-0 border-t border-slate-200 bg-white px-3 py-2 shadow-[0_-1px_6px_rgba(15,23,42,0.04)]">
          <Tooltip>
            ...
            >
              {criticalReadyModal ? (
                <p className="text-xs text-slate-700">Todos los datos críticos están completos.</p>
              ) : (
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-slate-500">
                    Requeridos: Cliente, Equipo, Marca, Falla y Revisión rápida.
                  </p>
      ...
      <ModalPatronSeguridad
        ...
        onSave={(encoded) => {
          ...
          toast({
            ...
            description: "La secuencia quedó registrada para este equipo.",
          })
        }}
      />
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-h-[calc(90vh-120px)] overflow-y-auto pr-6">
      ...
      <Card className="gap-0 py-0 overflow-hidden">
        ...
        <CardContent className="grid gap-4 p-5 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Input id="imei" name="imei" placeholder="Número IMEI o Serial" />
          </div>
          <div className="flex flex-col gap-2">
      ...
      <Card className="gap-0 py-0 overflow-hidden">
        ...
        <CardContent className="grid gap-4 p-5">
          <div className="flex flex-col gap-2">
            <Label htmlFor="problem-desc">Descripción del problema</Label>
            <Textarea
              id="problem-desc"
              name="problem-desc"
      ...
      <Card className="gap-0 overflow-hidden border-slate-200 bg-white py-0">
        <CardHeader className="border-b border-slate-100 py-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            PIN, contraseña o patrón para desbloqueo en taller
          </CardDescription>
        </CardHeader>
      ...
      <Card className="gap-0 py-0 overflow-hidden">
        <CardHeader className="border-b border-border py-4">
          ...
          <CardDescription>Opcional · máx. 3 fotos (se optimizan automáticamente)</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4 p-5">
          <input 
            ref={photoInputRef}
          ...
          >
            <div className="flex flex-col items-center gap-2 text-center">
              ...
              <p className="text-sm font-medium text-foreground">
                {compressingPhotos ? "Optimizando fotos…" : "Toca aquí para subir fotos"}
              </p>
              <p className="text-xs text-muted-foreground">
                Opcional · máx. 3 fotos · se optimizan automáticamente
              </p>
            </div>
          </button>
