...
interface ReparacionEditDialogProps {
  ...
  /** Solo edición de ticket existente */
  editingRepairId: string | null
  /** Tras guardar edición correctamente */
  onEditSuccess?: (repairId: string) => void
}

/**
 * Modal de edición de ticket (`NuevaReparacionForm` en modo edición).
 * Reutilizable desde la lista de reparaciones o la ficha por ID.
 *
 * Seguridad del equipo (PIN, contraseña, patrón) se edita en el colapsable «Seguridad»;
 * el **patrón de desbloqueo 3×3** se abre al elegir «Patrón» en Seguridad del equipo — mismo `ModalPatronSeguridad` que en el alta en modal.
 */
export function ReparacionEditDialog({
  ...
  const handleFormSubmit = async (
    ...
    const result = await createRepair({
      ...
      reportedFault: problemDesc,
      estimatedPrice,
      // El anticipo ya no se registra en caja desde la creación del ticket
      // El usuario puede registrar abonos después desde el módulo de caja
      clienteId: clienteId || undefined,
      technician: technician && technician !== "Sin asignar" ? technician : undefined,
      securityType,
      securityValue,
    ...
    if (!result.success || !result.repairId || !result.folio) {
      throw new Error(
        result.error?.trim() ||
          "El servidor no devolvió folio ni id. Revisa la consola de Vercel, migraciones de Supabase y columnas de `reparaciones`.",
      )
    }
