  ...
  const handleConfirmReingreso = useCallback(async () => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        toast({ variant: "destructive", title: "No se pudo reactivar", description: errMsg })
        return
      }
      toast({ title: "Reingreso activado y registrado en historial", description: `Folio ${repair.folio} reactiva por garantia.` })
      setReingresoOpen(false)
      setReingresoMotivo("")
      await load()
    } catch (e) {
      const raw = e instanceof Error ? e.message : String(e)
      setReingresoError(raw.trim() || "Error de red o del servidor al confirmar el reingreso.")
      toast({ variant: "destructive", title: "Error al confirmar", description: raw })
    } finally {
      ...
      <div className="min-h-0 w-full bg-slate-50">
        <div className="mx-auto flex w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          <div className="w-full max-w-2xl rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
            <p className="text-lg font-semibold tracking-tight text-slate-900">Esta reparacion no esta disponible.</p>
            <p className="mt-2 text-sm tracking-tight text-slate-500">Te estamos redirigiendo al listado.</p>
            <Button variant="outline" className="mt-6 rounded-xl border-slate-200 px-5 font-semibold text-slate-700 hover:bg-slate-50" onClick={handleBack}>
    ...
    <div className="min-h-0 w-full bg-slate-50">
      <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8 flex flex-col gap-4">
        ...
        {!PRO_FEATURES_TEMP_DISABLED && repair.estimatedPrice != null && (
          <MonitorUtilidadOperativa repairId={repair.id} folio={repair.folio} presupuesto={repair.estimatedPrice} initialGastos={gastos} />
        )}
      ...
      <ReparacionEditDialog open={editOpen} onOpenChange={setEditOpen} editingRepairId={editOpen ? repair.id : null} onEditSuccess={() => void load()} />
      <Dialog open={reingresoOpen} onOpenChange={(open) => { if (!reingresoLoading) setReingresoOpen(open) }}>
        <DialogContent className="max-w-sm rounded-2xl border-slate-200 bg-white p-0 gap-0 overflow-hidden shadow-lg">
          ...
          <div className="px-5 py-4 space-y-3">
            <div className="space-y-1.5">
              <Label className="text-[11px] font-semibold uppercase tracking-wide text-slate-600">Motivo del reingreso <span className="text-red-500">*</span></Label>
              <Textarea placeholder="Ej: El cliente reporta que el problema persiste despues de 3 dias de la entrega..." value={reingresoMotivo} onChange={(e) => { setReingresoMotivo(e.target.value); setReingresoError(null) }} className="resize-none text-sm min-h-[90px]" maxLength={500} disabled={reingresoLoading} />
              <p className="text-right text-[10px] text-slate-400">{reingresoMotivo.length}/500</p>
            </div>
            {reingresoError && <p className="text-xs text-red-600 font-medium">{reingresoError}</p>}
          </div>
          <DialogFooter className="flex-col sm:flex-row gap-2 px-5 py-4 border-t border-slate-100 bg-slate-50/50">
            <Button type="button" variant="outline" className="w-full sm:w-auto border-slate-200 text-slate-700" onClick={() => setReingresoOpen(false)} disabled={reingresoLoading}>Cancelar</Button>
            <Button type="button" className="w-full sm:flex-1 gap-2 bg-orange-600 hover:bg-orange-700 text-white font-bold" onClick={handleConfirmReingreso} disabled={reingresoLoading || reingresoMotivo.trim().length === 0}>
