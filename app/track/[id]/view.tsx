  ...
  const handleValidate = async (last4: string) => {
    ...
    try {
      ...
      if (!res.ok || !payload.ok || !payload.reparacion) {
        setError(payload.error ?? "Los últimos 4 dígitos no coinciden.")
        setTicket(null)
        setPhotoUrls([])
        setTallerName(null)
        setTallerLogo(null)
      ...
      console.error("Error validando reparación pública:", e)
      setError("Ocurrió un problema al validar tu información. Intenta de nuevo más tarde.")
      setTicket(null)
      setPhotoUrls([])
      setTallerName(null)
      setTallerLogo(null)
